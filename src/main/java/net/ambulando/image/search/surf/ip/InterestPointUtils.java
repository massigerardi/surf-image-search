...
public class InterestPointUtils {
    ...
    private final static double[][] GAUSS_25 = {
            ...
            { 0.00131955648461, 0.00103799989504, 0.00069579213743,
                    0.00039744277546, 0.00019345616757, 0.00008024231247,
                    0.00002836202103 }
    };
    final static double[][] GAUSS_33 = {
            { 0.014614763, 0.013958917, 0.012162744, 0.00966788, 0.00701053,
                    0.004637568, 0.002798657, 0.001540738, 0.000773799,
                    0.000354525, 0.000148179 },
            { 0.013958917, 0.013332502, 0.011616933, 0.009234028, 0.006695928,
                    0.004429455, 0.002673066, 0.001471597, 0.000739074,
                    0.000338616, 0.000141529 },
            { 0.012162744, 0.011616933, 0.010122116, 0.008045833, 0.005834325,
                    0.003859491, 0.002329107, 0.001282238, 0.000643973,
                    0.000295044, 0.000123318 },
            { 0.00966788, 0.009234028, 0.008045833, 0.006395444, 0.004637568,
                    0.003067819, 0.001851353, 0.001019221, 0.000511879,
                    0.000234524, 9.80224E-05 },
            { 0.00701053, 0.006695928, 0.005834325, 0.004637568, 0.003362869,
                    0.002224587, 0.001342483, 0.000739074, 0.000371182,
                    0.000170062, 7.10796E-05 },
            { 0.004637568, 0.004429455, 0.003859491, 0.003067819, 0.002224587,
                    0.001471597, 0.000888072, 0.000488908, 0.000245542,
                    0.000112498, 4.70202E-05 },
            { 0.002798657, 0.002673066, 0.002329107, 0.001851353, 0.001342483,
                    0.000888072, 0.000535929, 0.000295044, 0.000148179,
                    6.78899E-05, 2.83755E-05 },
            { 0.001540738, 0.001471597, 0.001282238, 0.001019221, 0.000739074,
                    0.000488908, 0.000295044, 0.00016243, 8.15765E-05,
                    3.73753E-05, 1.56215E-05 },
            { 0.000773799, 0.000739074, 0.000643973, 0.000511879, 0.000371182,
                    0.000245542, 0.000148179, 8.15765E-05, 4.09698E-05,
                    1.87708E-05, 7.84553E-06 },
            { 0.000354525, 0.000338616, 0.000295044, 0.000234524, 0.000170062,
                    0.000112498, 6.78899E-05, 3.73753E-05, 1.87708E-05,
                    8.60008E-06, 3.59452E-06 },
            { 0.000148179, 0.000141529, 0.000123318, 9.80224E-05, 7.10796E-05,
                    4.70202E-05, 2.83755E-05, 1.56215E-05, 7.84553E-06,
                    3.59452E-06, 1.50238E-06 }
    };

    /**
     * Returns the descriptor of the interest point as an array of 64 float
     * values. Depends on the <code>orientation</code> field of the interest
     * point. (I.e. in case of non-upright SURF the orientation must be computed
     * and assigned before.)
     */
    public static void computeAndSetDescriptor(InterestPoint ipt,
        ...
        while (i < 12) {
            ...
            while (j < 12) {
                ...
                len += (dx * dx + dy * dy + mdx * mdx + mdy * mdy) * gauss_s2
                        * gauss_s2;
    ...
    public static void computeAndSetOrientation(InterestPoint ipt,
            IntegralImage intImg) {
        ...
        for (int dx = -6; dx <= 6; dx++) {
            for (int dy = -6; dy <= 6; dy++) {
                if (dx * dx + dy * dy < 36) {
                    ...
                    resX[i] = gauss
                            * haarX(intImg, x + dx * s, y + dy * s, waveletSize); // <--
                                                                                  // Multiplication
                                                                                  // with
                                                                                  // scale
                                                                                  // value
                                                                                  ...
                                                                                  // here!
                    resY[i] = gauss
                            * haarY(intImg, x + dx * s, y + dy * s, waveletSize);
        ...
        // Calculate the dominant direction by looping over pi/3 sliding windows
        // (between the angles ang1 and ang2)
        // around the feature point
        final float windowSize = pi / 3; /*
                                          * The size of the sliding window (in
                                          * radians) used to assign an
                                          * orientation
                                          */
        final float step = 0.15f; /*
                                   * Increment used for the orientation sliding
                                   * window (in radians)
                                   */
        // ^^ OpenSURF: 0.15 radians (=>41.8 times), cvsurf: 5 degrees (=>72
        // times)
        float twoPi = 2 * pi;
        float sumX = 0, sumY = 0;
        ...
        for (float ang1 = 0, ang2 = ang1 + windowSize; ang1 < twoPi; ang1 += step, ang2 += step) {
            ...
            for (int k = 0; k < Ang.length; k++) {
                // Determine wheteher the point's angel is in between the angels
                // ang1 and ang2 (i.e. within the window):
                if ((ang1 <= Ang[k] && Ang[k] < ang2) ||
                        (ang1 <= Ang[k] + twoPi && Ang[k] + twoPi < ang2)) { // <--
                                                                             // because
                                                                             // we
                                                                             // can
                                                                             // be
            ...
            current = sumX * sumX + sumY * sumY; // <-- squared size of the
                                                 // vector
            if (current > max) {
                max = current;
    ...
    static float gaussian(float x, float y, float sig) {
        return (float) ((1 / (2 * pi * sig * sig)) * exp(-(x * x + y * y)
                / (2 * sig * sig)));
    }

    /**
     * Calculates Haar wavelet response in X direction at the point
     * <code>x</code>, <code>y</code> for the wavelet size <code>w</code>.
     */
