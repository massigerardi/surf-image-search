...
public class InterestPointsFinder {
    ...
    private final static int[][] FILTER_SIZES = { { 9, 15, 21, 27 },
            { 15, 27, 39, 51 }, { 27, 51, 75, 99 }, { 51, 99, 147, 195 } };
    private final static int[] MAX_FILTER_SIZES = { 27, 51, 99, 195 };

    private final static int STEP_INC_FACTOR = 2;
    private static int INIT_STEP = 2;// faster with 2
    ...
    public List<InterestPoint> findInterestingPoints(ImageProcessor processor) {
        return findInterestingPoints(processor, false);
    }

    public List<InterestPoint> findInterestingPoints(ImageProcessor processor, boolean normalized) {
        IntegralImage image = new IntegralImageImpl(processor, normalized);
        return findInterestingPoints(image);
    }

    public List<InterestPoint> findInterestingPoints(IntegralImage image) {
    ...
    private List<InterestPoint> fastHessian(IntegralImage img) {
        ...
        for (int octave = 0, step = INIT_STEP; octave < OCTAVES; octave++, step *= STEP_INC_FACTOR) {
            ...
            // Determine the border width (margin) for the largest filter in the
            // octave
            ...
            for (int layer = 0; layer < LAYERS; layer++) {
                int w = FILTER_SIZES[octave][layer]; // filter width == filter
                                                     // size
                int L = w / 3; // lobe size, e.g. 3 in 9x9 filter
                int L2 = 2 * L - 1; // "another lobe" size, e.g. 5 in 9x9 filter
                                    // (in Dxx and Dyy filters only)
                ...
                for (int y = margin; y < yBound; y += step) { // row
                    for (int x = margin; x < xBound; x += step) { // column
                        ...
                        // Normalise the filter responses with respect to their
                        // size
                        Dxx /= filterArea;
                        Dyy /= filterArea;
                        Dxy /= filterArea;
            ...
            // Adjust margins to get true 3x3 neighbors (with respect to the
            // actual 'step')
            margin += step;
            xBound -= step;
            yBound -= step;
            ...
            for (int layer = 1; layer < LAYERS - 1; layer++) {
                ...
                int filterSizeIncrement = filterSize
                        - FILTER_SIZES[octave][layer - 1];
                ...
                for (int y = margin; y < yBound; y += step) { // row
                    for (int x = margin; x < xBound; x += step) { // column
                        ...
                        // Interpolate maxima location within the 3x3x3
                        // neighborhood
                        ...
                        xInterp = x + X[0] * step;
                        yInterp = y + X[1] * step;
                        scale = (filterSize + X[2] * filterSizeIncrement)
                                * (1.2f / 9.0f);

                        // "Sometimes the interpolation step gives a negative size etc."
                        if (scale >= 1 && xInterp >= 0
                                && xInterp < img.getWidth() && yInterp >= 0
                                && yInterp < img.getHeight()) { // <-- from
                                                                // OpenCV-2.0.0
                            // ^^ should be OK instead of
                            // "if (abs(xi) < 0.5f && abs(xr) < 0.5f && abs(xc) < 0.5f)"
                            // (OpenSURF).
                            // The OpenCV-2.0.0 version leaves ~ 25% more IPs
                            res.add(new InterestPoint(xInterp, yInterp,
                                    det[layer][x][y], trace[layer][x][y], scale));
                        }
     ...
     */
    private static boolean isLocalMaximum(float v, float[][][] det, int s,
     ...
     */
    static float[] interpolatePoint(float[][][] det, int i, int x, int y,
            int step) {

        float[][] l = det[i - 1], m = det[i], u = det[i + 1]; // lower, middle
                                                              // and upper
                                                              // layers
        int px = x - step, nx = x + step, py = y - step, ny = y + step; // "previos"
                                                                        // x,
                                                                        // "next"
                                                                        // x,
                                                                        // "previos"
    ...
    public static boolean solve(float[][] A, float[] B, float[] X) {
        ...
        float a = A[0][0], b = A[0][1], c = A[0][2], d = A[1][0], e = A[1][1], f = A[1][2], g = A[2][0], h = A[2][1], i = A[2][2];

        float r = B[0], s = B[1], t = B[2];
    ...
    static float det(float a, float b, float c, float d, float e, float f,
