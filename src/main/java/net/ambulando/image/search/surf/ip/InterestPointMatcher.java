...
public class InterestPointMatcher {
    ...
    public static Map<InterestPoint, InterestPoint> findMatches(
            ...
            boolean doReverseComparisonToo, Heuristic heuristic) {
        Map<InterestPoint, InterestPoint> matchedPoints = InterestPointMatcher
                .findMatches(ipts1, ipts2, heuristic);

        if (doReverseComparisonToo) {
            Map<InterestPoint, InterestPoint> matchedPointsReverse = InterestPointMatcher
                    .findMatches(ipts2, ipts1, heuristic);
    ...
    public static Map<InterestPoint, InterestPoint> findMatches(
        ...
        for (InterestPoint p1 : ipts1) {
            ...
            intLoop: for (InterestPoint p2 : ipts2) {
                ...
                // Compare descriptors (based on calculating of squared distance
                // between two vectors)
            ...
            if (bestDistance < 0.5f * secondBest) {
                ...
                // res.size(), p1.x, p1.y, bestDistance, bestMatch.x,
                // bestMatch.y);
